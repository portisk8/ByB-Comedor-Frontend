...
function InfanteFormModal({ personaId, onClose }) {
  ...
  const personaListar = async () => {
    const response = await personaListarService({
      esInfante: false,
      comedorId: user.comedorId,
    });
  ...
  const handleValidateFechaNacimiento = (rule, value) => {
    ...
    try {
      if (edad > 18) {
        throw new Error("La persona debe ser menor de 18 años.");
      } else {
        Promise.resolve();
      }
    } catch (err) {
  ...
  return (
    ...
    >
        ...
        <Row gutter={16}>
          ...
          <Col xs={24}>
            ...
            <div style={{ marginTop: -20, width: "100%" }}>
              <a onClick={() => setTutorFormModal(true)}>Nuevo Tutor</a>
            </div>
          </Col>
        </Row>
      </Form>
      ...
      {tutorFormModal && (
        <TutorFormModal
          onClose={async (personaId) => {
            setTutorFormModal(false);
            if (personaId) {
              await personaListar();
              form.setFieldValue("personaTutorId", personaId);
            }
          }}
        />
      )}
...
export default InfanteFormModal;
